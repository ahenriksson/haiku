...
public:
	...
	fssh_status_t Init(size_t initialSize = kMinimumSize)
	{
	...
	fssh_status_t Insert(ValueType* value)
	{
		if (fTableSize == 0) {
	...
	void InsertUnchecked(ValueType* value)
	{
		if (CheckDuplicates && _ExhaustiveSearch(value)) {
			fssh_panic("Hash Table: value already in table.");
		}
	...
	bool RemoveUnchecked(ValueType* value)
	{
		...
		if (CheckDuplicates && _ExhaustiveSearch(value)) {
			fssh_panic("Hash Table: duplicate detected.");
		}
